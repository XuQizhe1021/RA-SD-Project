...
- `frontend/src/views/EvaluationView.vue`：评价管理页面
- `frontend/src/views/StatisticsView.vue`：统计报表页面
...
- 新增 `评价管理` 后端接口与前端页面，覆盖学员自评、现场工作人员代录、经理/执行人评价汇总与课程评价报告
- 新增 `统计报表` 后端接口与前端页面，覆盖按课程、按学员、按讲师、收入汇总四类查询
- 补齐评价表结构字段 `source`、`proxy_staff_id`，保证“学员自评 / 现场代录”可区分展示
- 优化企业统付收费口径，企业课程初始化时同步写入应收与实收，保证收入统计结果准确
- 初始化脚本补充了报名、签到、收费、评价演示数据，导入后可直接演示 Day12 主流程与 Day13 评审统计
...
- 进入 `收费管理`
- 查看收费列表并执行收费
- 使用 `student02 / 123456` 登录，进入 `评价管理` 提交课程评价
- 使用 `staff01 / 123456` 登录，进入 `评价管理` 查看待代录名单并演示现场代录
- 使用 `manager01 / 123456` 或 `executor01 / 123456` 登录，进入 `评价管理` 查看课程评价汇总与单课程报告
- 进入 `统计报表`，查看课程、学员、讲师、收入四个维度的统计结果
- 将课程管理、讲师管理、报名管理、签到管理、收费管理、评价管理、统计报表页面截图提供给 E 归档
...
| 角色 | 可访问模块 | 主要能力 |
| --- | --- | --- |
| 经理 | 首页概览、培训申请、课程管理、讲师管理、评价管理、统计报表 | 查看培训申请、掌握课程计划与讲师资源、查看评价结果、跟踪经营统计 |
| 执行人 | 首页概览、课程管理、讲师管理、学员管理、通知发布、报名管理、评价管理、统计报表 | 维护课程与讲师、发布通知、处理报名审核、查看评价汇总、支撑培训执行 |
| 现场工作人员 | 首页概览、签到管理、收费管理、评价管理 | 核验报名名单、执行签到与收费、代录与整理培训评价 |
...
签到功能已按需求规约中的培训现场流程接入数据库，角色边界如下：

| 角色 | 是否拥有签到模块 | 权限边界 |
| --- | --- | --- |
| 经理 | 否 | 不直接参与签到，通过统计报表关注课程到场情况 |
| 执行人 | 否 | 不执行签到，负责提前完成课程、通知、报名审核等前置工作 |
| 现场工作人员 | 是 | 查看已确认报名名单、执行签到、填写现场备注 |
| 学员 | 否 | 不进入签到管理页面，只在培训现场由工作人员核验并完成签到 |

签到功能的数据流转如下：

1. 执行人在报名管理中审核通过报名；
2. 后端自动为该报名初始化 `attendance_record`；
3. 现场工作人员进入 `签到管理` 查询待签到名单；
4. 执行签到后，系统写入 `checked_in_at`、`checked_in_by`、`remark`；
5. 操作日志同步记录到 `operation_log`，供后续审计和过程材料使用。
...
同时包含 `课程管理`、`讲师管理`、`报名管理`、`签到收费联调`、`评价管理`、`统计报表` 所需的演示种子数据，导入后可直接登录前端查看真实库记录。
...
6. 打开 `签到管理` 并执行签到
7. 打开 `收费管理` 并执行收费
8. 使用 `student02` 打开 `评价管理`，确认存在 `1` 条待评价课程
9. 使用 `manager01` 打开 `评价管理`，确认可查看课程评价汇总与详情
10. 使用 `manager01` 打开 `统计报表`，确认收入汇总显示 `2598.00`
11. 确认课程列表有 `2` 条数据
12. 确认讲师列表有 `2` 条数据
